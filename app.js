(function () {
  ...
  // 👉 v8.4 生态模式一键预设与中枢引擎
  window.setEcologyMode = function (mode) {
    var cbs = {
      hide02adv: document.getElementById('eco_hide02adv'),
      hide02groups: document.getElementById('eco_hide02groups'),
      hide01filter: document.getElementById('eco_hide01filter'),
      hide01rituals: document.getElementById('eco_hide01rituals'),
      hide01pill: document.getElementById('eco_hide01pill'),
      hide03cabinet: document.getElementById('eco_hide03cabinet'),
      hide03heatmap: document.getElementById('eco_hide03heatmap'),
      hide03cold: document.getElementById('eco_hide03cold'),
      hide00window: document.getElementById('eco_hide00window'),
      hide00journal: document.getElementById('eco_hide00journal')
    };
    ...
    if (mode === 'standard') {
      ...
      if (typeof showToast === 'function') showToast('⚖️ 已切换至标准航海模式');
    } else if (mode === 'muggle') {
      if (cbs.hide02adv) cbs.hide02adv.checked = true;
      if (cbs.hide02groups) cbs.hide02groups.checked = true;
      if (cbs.hide01filter) cbs.hide01filter.checked = true;
      if (cbs.hide01rituals) cbs.hide01rituals.checked = false;
      if (cbs.hide01pill) cbs.hide01pill.checked = true;
      if (cbs.hide03cabinet) cbs.hide03cabinet.checked = true;
      if (cbs.hide03heatmap) cbs.hide03heatmap.checked = true;
      if (cbs.hide03cold) cbs.hide03cold.checked = true;
      if (cbs.hide00window) cbs.hide00window.checked = true;
      if (cbs.hide00journal) cbs.hide00journal.checked = false;
      if (typeof showToast === 'function') showToast('🧩 已切换至极简麻瓜模式');
    } else if (mode === 'sanctuary') {
      Object.keys(cbs).forEach(function (k) {
        var cb = cbs[k];
        if (cb) cb.checked = true;
      });
      if (typeof showToast === 'function') showToast('🩸 已开启避风港模式');
    }
    if (typeof toggleEcology === 'function') toggleEcology();

    if (mode === 'muggle' || mode === 'sanctuary') {
      document.body.classList.add('muggle-dashboard');
      if (typeof anchorSwiper !== 'undefined' && anchorSwiper) anchorSwiper.slideTo(1, 300);
      setTimeout(function () {
        if (typeof outerSwiper !== 'undefined' && outerSwiper && typeof outerSwiper.disable === 'function') outerSwiper.disable();
        if (typeof innerSwiper !== 'undefined' && innerSwiper && typeof innerSwiper.disable === 'function') innerSwiper.disable();
      }, 350);
    } else {
      document.body.classList.remove('muggle-dashboard');
      if (typeof outerSwiper !== 'undefined' && outerSwiper && typeof outerSwiper.enable === 'function') outerSwiper.enable();
      if (typeof innerSwiper !== 'undefined' && innerSwiper && typeof innerSwiper.enable === 'function') innerSwiper.enable();
    }

    try {
      if (mode === 'muggle' || mode === 'sanctuary') {
        localStorage.setItem('anchor_ecology_dashboard', mode);
      } else if (mode === 'standard') {
        localStorage.removeItem('anchor_ecology_dashboard');
      }
    } catch (eDash) {}
  };

  window.toggleEcology = function () {
    var prefs = {};
    document.querySelectorAll('.eco-cb').forEach(function (cb) {
      prefs[cb.id] = cb.checked;
      document.body.classList.toggle('eco-' + cb.id, cb.checked);
    });

    var is02Empty = prefs.eco_hide02adv && prefs.eco_hide02groups;
    document.body.classList.toggle('eco-center-02', is02Empty);

    try {
      localStorage.setItem('anchor_ecology_prefs', JSON.stringify(prefs));
    } catch (e) {}
  };

  window.initEcology = function () {
    try {
      var saved = JSON.parse(localStorage.getItem('anchor_ecology_prefs') || '{}');
      document.querySelectorAll('.eco-cb').forEach(function (cb) {
      ...
      try {
        var dash = localStorage.getItem('anchor_ecology_dashboard');
        if (dash === 'muggle' || dash === 'sanctuary') {
          document.body.classList.add('muggle-dashboard');
          if (typeof anchorSwiper !== 'undefined' && anchorSwiper) anchorSwiper.slideTo(1, 0);
          setTimeout(function () {
            if (typeof outerSwiper !== 'undefined' && outerSwiper && typeof outerSwiper.disable === 'function') outerSwiper.disable();
            if (typeof innerSwiper !== 'undefined' && innerSwiper && typeof innerSwiper.disable === 'function') innerSwiper.disable();
          }, 350);
  ...
  window.openMuggleRecord = function () {
    if (typeof renderList === 'function') renderList();
    if (typeof renderHistory === 'function') renderHistory();

    var destPending = document.getElementById('mugglePendingList');
    var srcDaily = document.getElementById('dailyList');
    var srcMedia = document.getElementById('mediaList');
    if (destPending) {
      destPending.innerHTML = (srcDaily ? srcDaily.innerHTML : '') + (srcMedia ? srcMedia.innerHTML : '');
    }

    var destHistory = document.getElementById('muggleHistoryList');
    var srcHistory = document.getElementById('historyListContainer');
    if (destHistory && srcHistory) destHistory.innerHTML = srcHistory.innerHTML;

    var recModal = document.getElementById('muggleRecordModal');
...
skipBtn.style.display = 'block';
  ...
  function openCandyEngine() {
    ...
    if (document.body.classList.contains('muggle-dashboard') && section) {
  ...
  function closeCandyEngine() {
    ...
    if (document.body.classList.contains('muggle-dashboard') && section) {
      var zone = document.getElementById('onboardingCandyZone');
      var stats = document.getElementById('onboardingStatsZone');
      if (zone && stats) {
        zone.insertBefore(section, stats);
      } else {
        var shell = document.querySelector('.sanctuary-shell');
        if (shell) shell.appendChild(section);
      }
    }
  }
  ...
  // 👉 核心修复：不再使用游离的 new Audio()，直接抓取 DOM 物理标签，跟随页面销毁而强制释放硬件管道
  const bubblePopAudio = document.getElementById('domAudioBubble');
  if (bubblePopAudio) {
  ...
  const fireworkPopAudio = document.getElementById('domAudioFirework');
  if (fireworkPopAudio) fireworkPopAudio.volume = 1.0;

  const starShineAudio = document.getElementById('domAudioStar');
  if (starShineAudio) starShineAudio.volume = 0.65;
  ...
  // 👉 v8.4 终极音效引擎：iOS PWA 物理重载与全域同步解锁
  window.unlockAllCandySounds = function() {
    var btn = document.getElementById('candySoundBtn');
    var ctx = getCandyAudioCtx();

    // 1. 苹果铁律：同步状态下直接 resume
    if (ctx && ctx.state === 'suspended') {
      ctx.resume().catch(function () {});
    }

    // 2. Web Audio 空炮：打通内存合成音通道 (乐器糖)
    try {
      if (ctx) {
      ...
      console.warn('空音频唤醒失败', e);
    }

    // 3. 核心修复：模拟「切出再切回」的硬件重置 (Hardware Reload)
    function hardReloadAndKick(audioEl) {
      if (!audioEl) return;
      try {
        audioEl.load();
    ...
    // 4. 对所有需要 HTML5 播放的实例执行硬件重载
    if (typeof bubblePopAudio !== 'undefined') hardReloadAndKick(bubblePopAudio);
    if (typeof fireworkPopAudio !== 'undefined') hardReloadAndKick(fireworkPopAudio);
    if (typeof starShineAudio !== 'undefined') hardReloadAndKick(starShineAudio);
    ...
    if (typeof updateCandySoundButton === 'function') {
      updateCandySoundButton(typeof currentCandyNeedsSound !== 'undefined' ? currentCandyNeedsSound : false);
    }

    if (typeof showToast === 'function') showToast('🎵 全域音效引擎已重载并就绪');
  };
  ...
  // pagehide 时由文件末尾的全局监听器调用；必须挂 window 才能访问 IIFE 内的 _candyAudioCtx 与 HTML5 实例
  window._anchorReleaseCandyAudioForPagehide = function () {
    try {
      if (typeof _candyAudioCtx !== 'undefined' && _candyAudioCtx && _candyAudioCtx.state !== 'closed') {
        if (typeof _candyAudioCtx.close === 'function') {
          _candyAudioCtx.close().catch(function () {});
        }
      }
      _candyAudioCtx = null;

      var releaseHardwareAudio = function (audioEl) {
        if (audioEl) {
          audioEl.pause();
      ...
      if (typeof bubblePopAudio !== 'undefined') releaseHardwareAudio(bubblePopAudio);
      if (typeof fireworkPopAudio !== 'undefined') releaseHardwareAudio(fireworkPopAudio);
      if (typeof starShineAudio !== 'undefined') releaseHardwareAudio(starShineAudio);
    } catch (e) {
      console.warn('音频锁释放异常', e);
    }
  };
  ...
  function playBubblePopSound() {
    ...
    if (!bubblePopAudio) return;
    try {
      bubblePopAudio.currentTime = 0;
      var p = bubblePopAudio.play();
  ...
  function playFireworkPopSound() {
    if (isStandaloneMode() && !soundUnlocked) return;
    if (!fireworkPopAudio) return;
    try {
      fireworkPopAudio.currentTime = 0;
      var p = fireworkPopAudio.play();
      if (p && typeof p.catch === 'function') p.catch(function() {});
    } catch(e) {}
  }
  ...
  function stopStarShineSound() {
    try {
      if (starShineAudio) starShineAudio.pause();
    } catch (e) {}
  }

  function onCandySoundBtnClick(ev) {
    if (ev) {
      ev.preventDefault();
      ev.stopPropagation();
    }
  ...
  function initCandySoundButton() {
    var btn = document.getElementById('candySoundBtn');
    if (!btn) return;
    btn.removeEventListener('click', onCandySoundBtnClick);
    btn.addEventListener('click', onCandySoundBtnClick, false);
  }
  ...
  function renderBubbleCandy(stageEl) {
    ...
    bubbles.forEach(function (btn) {
      btn.addEventListener('click', function () {
        try { var _ac = getCandyAudioCtx(); if (_ac && _ac.state === 'suspended') _ac.resume(); } catch (_) {}
  ...
  function updateUsernameUI() {
    var displayName = (typeof currentUsername !== 'undefined' && currentUsername && currentUsername !== 'Anchor') ? currentUsername : '你';
    var posterUname = document.getElementById('dailyPosterUname');
    if (posterUname) posterUname.textContent = displayName;
    const muggleTop = document.getElementById('muggleTopUname');
    if (muggleTop) {
      muggleTop.textContent = (typeof currentUsername !== 'undefined' && currentUsername && currentUsername !== 'Anchor') ? currentUsername : 'Anchor';
    }
  }
...
window.toggleSettingTimer = function(checked) {
  ...
  async function completeTaskCore(task, opts) {
    ...
    if (task && task.type === 'sos') {
      if (!silent) {
        ...
        if (btnPauseSos) { btnPauseSos.style.display = 'none'; btnPauseSos.innerText = '⏸️'; }
 ...
 if (finalTask.projectId && typeof customPlaylists !== 'undefined') {
  ...
  if (pIdx > -1) {
    ...
    if (!silent && isProgressFull) {
      ...
      if (btnPauseFull) { btnPauseFull.style.display = 'none'; btnPauseFull.innerText = '⏸️'; }
    ...
    if (btnPauseComplete) { btnPauseComplete.style.display = 'none'; btnPauseComplete.innerText = '⏸️'; }
    if (typeof renderLog === 'function') renderLog();
    if (typeof renderHistory === 'function') renderHistory();
 ...
 // 👉 What & Why: 找回 v7.1.4 金标准。仅静默同步底层清单的 done 状态。
  ...
  if (typeof dailyLog === 'undefined' || !Array.isArray(dailyLog)) { 
    ...
    if (typeof initEcology === 'function') initEcology();
...
`;
  ...
  function resurrectHistoryTask() {
    ...
    var muggleModal = document.getElementById('muggleRecordModal');
    if (muggleModal) muggleModal.style.display = 'none';
    const dlg = document.getElementById('historyTaskDialog');
    if (dlg) dlg.style.display = 'none';
  ...
  function openTaskEditorModal(taskObj, projectId) {
    ...
    var modalInp = document.getElementById('modalInpTitle');
    if (modalInp) {
      modalInp.placeholder = document.body.classList.contains('eco-eco_hide02adv')
        ? '随便写点什么...系统会自动帮你分类...'
        : '输入任务描述...';
    }
...
window.lobbyEnter = function(action) {
  ...
  setTimeout(() => {
      if (action === 'candy') {
          if (!document.body.classList.contains('muggle-dashboard') && typeof anchorSwiper !== 'undefined') {
            anchorSwiper.slideTo(0);
          }
...
// ============================================================
// 👉 v8.4 PWA 终极救星：页面刷新前的「释放硬件锁」(Garbage Collection)
// 解决 iOS Safari/PWA 刷新不释放 AudioContext 导致并发配额耗尽的静音 Bug
// 实际释放逻辑在 IIFE 内 window._anchorReleaseCandyAudioForPagehide（可访问闭包变量）
// ============================================================
window.addEventListener('pagehide', function () {
    try {
        if (typeof window._anchorReleaseCandyAudioForPagehide === 'function') {
            window._anchorReleaseCandyAudioForPagehide();
        }
    } catch (e) {
        console.warn('音频锁释放异常', e);
    }
